...
## Example:
### Simple:
If all you want is to generate a chess board image, based on a FEN string, then all you have to do is the following:
...
json, you can do that by either edit the default.json, or by creating the styles manually. 
For example, let's change the highlight color and style, the annotation position and border color and the move size and 
color for the next image. And for fun, lets change the render order too...

<img src="advanced.png" alt="drawing" width="350"/>

As you can see, the pieces are now rendered **after** annotations.

This chess board image can be generated by using the following code:
```go
	imager := chessImager.NewImager()
	...
	// Highlight e7 square, with a circle of radius 35
	hs, _ := ctx.NewHighlightStyle(chessImager.HighlightCircle, "#88E57C", 35)
	ctx.AddHighlightEx("e7", hs)

	// Annotate square e7 as a brilliant move (!!)
	...
	// Show move e1-e7 with smaller dots in a different color
...
either use the embedded default.json or you can create your own configuration file. You can load your own config 
...
* ctx.AddHighlight().AddMove().AddAnnotation()
* Should NewContext be a method on chessImager?
